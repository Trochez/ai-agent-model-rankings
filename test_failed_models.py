#!/usr/bin/env python3
"""
Re-test only models that failed in the last test execution.
Based on direct-api-test-results-20260406_150732.json
"""
import json
import os
import time
...
# Models that FAILED in the last test (from direct-api-test-results-20260406_150732.json)
FAILED_MODELS = [
    {"id": "openai/gpt-5.4", "provider": "openai", "error": "HTTP 429: quota exceeded"},
    ...
    {"id": "openai/gpt-5.3-codex", "provider": "openai", "error": "HTTP 429: quota exceeded"},
...
def main():
    print("=" * 80)
    print("RE-TEST FAILED MODELS FROM LAST EXECUTION")
    print(f"Started: {datetime.now()}")
    print(f"Models to retest: {len(FAILED_MODELS)}")
    print("=" * 80)
    ...
    results = []
    for i, model_info in enumerate(FAILED_MODELS, 1):
        print(f"\n[{i}/{len(FAILED_MODELS)}]", end="")
    ...
    results_file = RESULTS_DIR / f"failed-models-retest-{timestamp}.json"
    report_file = RESULTS_DIR / f"failed-models-retest-report-{timestamp}.md"
    
    with open(results_file, 'w') as f:
        json.dump({
            "test_date": datetime.now().isoformat(), 
            "test_prompt": TEST_PROMPT, 
            "test_type": "retest_failed_models",
            "results": results
        }, f, indent=2)
    ...
    report_lines = [
        f"# Failed Models Re-Test Report",
        f"**Date**: {datetime.now()}",
        f"**Test**: '{TEST_PROMPT}' → Expected: '4'",
        f"**Models Retested**: {total} (from direct-api-test-results-20260406_150732.json)",
        f"",
